...
uteam=input('Enter your team : ')
team=uteam+"_vs_computer"

# Toss 
 
print("\nHere comes the Toss")
toss = (input("Choose heads or tails(Type h/t): ")).lower()
...
if runs_1 > runs_2:
    ...
    if Bat_first == "You": 
        print(f"\nCongratulations! {uteam} won the Match by",runs_1 - runs_2,"runs.")
    ...
    if Bat_second == "You": 
        print(f"\nCongratulations! {uteam} won the Match by",2 - wickets_2,"wickets.")
...
#edited
print("(Player,Runs,Wicket)")
mycon=s.connect(host="localhost",user="root",passwd="cs208@08",database="cric")
cursor=mycon.cursor()
query='Create table '+team+' (Player char(30),Runs int, Wicket int)'
cursor.execute(query)
q='insert into '+team+' (Player, Runs, Wicket) values (%s,%s,%s)'
L=[(Bat_first,runs_1, wickets_1),(Ball_first,runs_2, wickets_2)]
for i in L:
    cursor.execute(q,i)
mycon.commit()
...
for i in data:
    print(i)
print("Please view the table in MYSQL.....\n")
j=input("Do You want to clear the table, else the same team can't be repeated(y/n)")
if j=='y':
